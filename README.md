...
The homepage was shipping raw CSS as visible text, crystals weren't
rendering, and buttons showed default Streamlit styling instead of the
pill design. Root cause: `st.markdown(..., unsafe_allow_html=True)` routes
content through Streamlit's markdown parser first, which has a documented
failure mode (streamlit/streamlit#586, #5868, #859) where a blank line
partway through a long injected `<style>`/`<div>` block causes everything
after it to be dumped out as literal text instead of applied as HTML/CSS --
exactly what showed up in the screenshot. Fixed by switching every
pure-HTML/CSS injection, site-wide, to `st.html()` (skips the markdown
parser entirely) and replacing the one "open a div, render widgets, close
it in a later call" trick with `st.container(key=...)`, Streamlit's own
supported mechanism for styling a wrapper around real widgets.
