...
def predict_cleavage_sites(profile_chain, rsa_threshold=0.45, cluster_window=3):
    ...
    Two things matter here that the first version of this function got
    wrong (confirmed against real cached structure data: it was producing
    strings of near-identical single-residue "fragments" from what was
    clearly one contiguous exposed loop, e.g. four separate ~146 Da
    fragments in a row):

    1. Threshold raised to 0.45. "Limited" proteolysis is limited by
       design -- the reaction is stopped early specifically so only the
       most kinetically favored (most exposed) backbone positions get
       cut, leaving large, well-separated fragments. The original 0.2
       threshold (a generic bioinformatics "exposed" cutoff) flagged far
       too many moderately-exposed residues as independent cut events,
       producing a step-ladder of tiny fragments no real gel or MS run
       would ever show.
    2. Clustering: adjacent exposed residues within cluster_window of
       each other are very likely part of the SAME flexible loop, not
       independent cleavage events. Only the single most-exposed residue
       within each cluster is kept as the representative cut site.

    This is still a first-order approximation (see module docstring) --
    but it now reflects the "handful of dominant cuts" behavior real
    limited digestion actually produces, not exhaustive digestion.
    """
    candidates = []
...
def generate_fragment_ladder(profile_chain, cleavage_sites):
    ...
    compute each fragment's average mass. Returns a list of dicts:
    {start, end, length, mass_da}, sorted by mass descending (matching
    how a gel or MS spectrum is typically read, largest first)."""
...
def predicted_ladder_for_structure(pdb_id, mode="full-core", rsa_threshold=0.45, db_path="sg_atlas_cache.db"):
    ...
    ladder for its best (highest-degree, i.e. most representative) chain.
    Returns None if the structure isn't in the cache yet."""
    ...
    # use whichever chain has the most residues resolved -- the most
    # complete, representative picture of this structure's fold
...
def compute_match_confidence(n_trials, n_matches, chance_probability):
    ...
    "3 out of 4 matched" sounds similar whether those 4 trials were highly
    specific (unlikely to align by chance) or nearly guaranteed to align
    anyway. This asks the real question: given how likely a chance
    alignment is (chance_probability), how surprising is it that we saw
    n_matches out of n_trials? That's a binomial enrichment test -- the
    same style of test used in bioinformatics for "is this overlap more
    than expected by chance" (e.g. genomic feature enrichment).

    Returns (p_value, confidence) where confidence = 1 - p_value.
    p_value = probability of seeing n_matches OR MORE purely by chance.
    A low p-value (high confidence) means this match pattern would be
    unlikely to happen randomly -- i.e. it's real structural signal, not
    coincidence.
    """
    if n_trials == 0:
        return 1.0, 0.0  # no evidence at all -- can't be confident of anything
...
def match_fragments_by_position(observed_fragments, pdb_id, mode="full-core", tolerance_window=2,
    ...
    """
    The precision-matching mode: instead of comparing fragment MASSES
    (which can coincidentally align between totally different cut
    patterns), this compares actual cleavage POSITIONS -- what real
    MS/MS peptide sequencing gives you, not just intact peptide mass.

    observed_fragments: list of (start, end) residue-number tuples, e.g.
    [(38, 61), (62, 89), (90, 97)] -- the real sequence-identified
    fragments from a patient sample's digest.

    For each fragment boundary that implies an actual cleavage event (not
    just the natural end of the resolved chain), checks whether this
    structure's predicted cleavage sites include something within
    tolerance_window residues. This is a direct, unambiguous positional
    check -- no mass-coincidence ambiguity possible.

    Returns None if the structure isn't cached, otherwise a dict with the
    match detail AND a statistically grounded confidence score.
    """
    conn = sqlite3.connect(db_path)
    cur = conn.execute(
        "SELECT chain_id, residue, amino_acid, sasa FROM residues WHERE pdb_id=? AND mode=?",
    ...
    for start, end in observed_fragments:
        for boundary, label, is_natural_end in [
            ...
            if is_natural_end:
                continue  # not a cleavage event -- it's just the edge of what's resolved
            trials += 1
    ...
    # chance probability: how much of the resolved chain falls within
    # tolerance_window of SOME predicted site, if a boundary landed at
...
def match_observed_fragments_by_position(observed_fragments, tolerance_window=2, db_path="sg_atlas_cache.db"):
    """Run position-based matching against every cached structure, ranked
    by confidence (statistical significance), not raw match count."""
    conn = sqlite3.connect(db_path)
    ...
    for pdb_id in all_pdb_ids:
        r = match_fragments_by_position(observed_fragments, pdb_id, tolerance_window=tolerance_window,
                                          db_path=db_path)
...
def theoretical_max_fragment(profile_chain):
    """The largest possible fragment size for this chain -- i.e. what
    you'd get with zero cuts. Since cryo-EM fibril structures only
    resolve the ordered core (disordered flanking tails aren't modeled
    at all), this is a hard ceiling: no predicted fragment, and no
    honest match against an observed mass, can ever legitimately exceed
    this value for this structure."""
...
def match_observed_fragments(observed_masses, tolerance_pct=5.0, rsa_threshold=0.45, db_path="sg_atlas_cache.db"):
    ...
    researcher actually observed (from SDS-PAGE or MS), score every
    cached structure's predicted ladder by how well it explains them,
    and return results ranked best-match first.

    Match score = fraction of observed masses that have some predicted
    fragment within tolerance_pct% -- a simple, transparent, explainable
    metric rather than a black-box score, since a lab needs to be able to
    see WHY a match was scored the way it was.

    IMPORTANT: an observed mass that exceeds this structure's theoretical
    maximum (the full resolved chain, zero cuts) is marked "exceeds_max"
    rather than compared against whatever fragment happens to be closest.
    That's a real, hard ceiling from how much sequence the cryo-EM
    structure actually resolves (disordered flanking tails are invisible
    to cryo-EM and never appear in these coordinates) -- silently
    reporting a "closest match" percentage against an impossible target
    is actively misleading, not just imprecise. These observations are
    excluded from the match score entirely (not counted as either a
    match or a failure) rather than being folded in as false negatives.
    """
    conn = sqlite3.connect(db_path)
    cur = conn.execute("SELECT DISTINCT pdb_id FROM structures WHERE mode='full-core'")
    all_pdb_ids = [row[0] for row in cur.fetchall()]
    ...
    for pdb_id in all_pdb_ids:
        ...
        max_possible = max(predicted_masses)  # the zero-cut fragment, if present, is always the largest

        matched = 0
        evaluable = 0
        match_details = []
        for obs in observed_masses:
            if obs > max_possible * 1.05:  # small tolerance for rounding, not a real allowance
        ...
        # score is only computed over observations this structure could
        # theoretically explain at all -- an observation that exceeds
        # every structure's max equally shouldn't be silently penalizing
    ...
    # IMPORTANT: sort by evaluable_count FIRST, score second. A structure
    # that could only be tested against 1 of 3 observed masses and
    # "matched" it isn't more trustworthy than one tested against all 3
    # and matched 2 -- it's LESS informative, since most of the real
    # evidence couldn't even be checked against it. Ranking by raw score
    # alone rewards small/poorly-resolved structures for dodging most of
    # the data, which is backwards. Coverage comes first; match quality
    # breaks ties within comparable coverage.
