...
def inject_global_css():
    ...
    st.markdown(
        ...
        .sg-nav-wrap [data-testid="stButton"] button {{
        ...
        .sg-nav-wrap [data-testid="stButton"] button:hover {{
            opacity: 0.6;
            transform: translateY(-1px);
        }}
        .sg-nav-wrap [data-testid="stButton"] button:focus-visible {{
        ...
        .sg-nav-wrap [data-testid="stButton"] button:active,
...
def render_top_nav(pages, active=None):
    ...
    you're on)."""
    st.markdown('<div class="sg-nav-wrap">', unsafe_allow_html=True)
    cols = st.columns(len(pages))
    for col, (label, target) in zip(cols, pages.items()):
        with col:
            # use_container_width=True so each label centers within its own
            # equal-width column (left/middle/right thirds) -- the simplest
            # reliable way to get the reference image's evenly-spaced,
            # spread-wide nav layout out of Streamlit's real button widget.
