...
# Global CSS (badges/cards used across result views). Was previously only
# injected inside the "no mass results" branch, which meant it silently
# never loaded whenever a search actually returned matches -- moved here
# so it's always available regardless of mode or outcome.
st.markdown("""
<style>
    /* Metric Cards */
    .metric-card {
...
if mode.startswith("Precision"):
    ...
    if run:
        ...
        if observed:
            pass  # TODO: mass-based scoring is not wired up yet -- results stays empty.
            # When there's a mass-matching equivalent of score_all_candidates in
            # sg_atlas_fragments.py, call it here, e.g.:
            #   results = score_all_candidates_by_mass(
            #       observed, tolerance_pct=tolerance_pct, db_path=CACHE_DB,
            #   )
